...
/*global jQuery */
/*Modify by Aiska Hendra on 2013-04-12 */
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setColWidth();
		$(eg).css("width", (ts.p.autowidth === true ? "" : grid.width + "px")).append("<div class='ui-jqgrid-resize-mark' id='rs_m" + ts.p.id + "'>&#160;</div>");
		$(gv).css("width", (ts.p.autowidth === true ? "" : grid.width + "px"));
		thead = $("thead:first", ts).get(0);
		var	tfoot = "";
		if (ts.p.footerrow) { tfoot += "<table role='grid'" + (ts.p.autowidth === true ? "" : " style='width:" + ts.p.tblwidth + "px'") + " class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-" + dir + "'>"; }
		...
		$(grid.hDiv)
			.css({ width: (ts.p.autowidth === true ? "" : grid.width + "px") })
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({ width: (ts.p.autowidth === true ? "" : grid.width + "px") }).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
		...
		$(grid.bDiv)
            ...
            .css({ height: ts.p.height + (isNaN(ts.p.height) ? "" : "px"), width: (ts.p.autowidth === true ? "" : (grid.width) + "px") })
